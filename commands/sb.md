...
The TUI auto-detects the `claude` pane in the current tmux window. If detection picks the wrong pane (or nothing), use `/sb-attach <pane_id>` to override and `/sb-detach` to clear.
...
If the printed line says "no claude pane detected", suggest `/sb-attach <pane_id>`. `/sb-web` opens the browser fallback if they prefer that.
